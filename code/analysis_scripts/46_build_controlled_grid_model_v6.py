#!/usr/bin/env python3
"""Build controlled grid model V6.

V6 tests whether the v5 emergence/suppression pattern is merely a byproduct of
...
OUT_GRID = PROJECT_ROOT / "data" / "derived_data" / "model" / "controlled_grid_model_500m_2025_v6.csv"
OUT_METRICS = PROJECT_ROOT / "data" / "derived_data" / "model" / "controlled_grid_model_metrics_v6.csv"
OUT_COEFS = PROJECT_ROOT / "data" / "derived_data" / "model" / "controlled_grid_model_coefficients_v6.csv"
REPORT = PROJECT_ROOT / "_rebuild_outputs" / "reports" / "62_controlled_grid_model_v6_report.md"
...
def build_frame() -> pd.DataFrame:
    ...
    df["is_suppressed_frontier"] = (df["grid_emergence_type"] == "suppressed_emergence_frontier").astype(int)
...
def main() -> None:
    ...
    for target in ["is_suppressed_frontier", "has_positive_rule", "has_primary_positive_rule"]:
        ...
        if not coefs.empty:
            ...
            df[f"pred_{target}_v6"] = pred
    metrics, coefs, pred = fit_ridge_cv(df, "suppression_index", full_numeric, CATEGORICAL)
    metrics["spec"] = "core_plus_controls"
    metrics_rows.append(metrics)
    coef_frames.append(coefs.assign(spec="core_plus_controls"))
    df["pred_suppression_index_v6"] = pred
    ...
    key_cols = [
        ...
        "pred_is_suppressed_frontier_v6",
        "pred_has_positive_rule_v6",
        "pred_has_primary_positive_rule_v6",
        "pred_suppression_index_v6",
        *CORE_NUMERIC,
        *CONTROL_NUMERIC,
        "primary_source_support_class",
    ...
    REPORT.write_text(
        f"""# 62 Controlled Grid Model V6 Report
...
V6 adds morphology, boundary/area, district fixed context, host venue-type mix
